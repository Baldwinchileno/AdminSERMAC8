...
namespace AdminSERMAC.Forms
{
    public class MainForm : Form
    {
        ...
        public MainForm(IClienteService clienteService, string connectionString, ILogger<MainForm> logger, ILogger<SQLiteService> sqliteLogger)
        {
        ...
        private void MostrarInventarioButton_Click(object sender, EventArgs e)
        {
            try
            {
                var inventarioForm = new InventarioForm(_sqliteLogger);
                inventarioForm.Show();
            }
            catch (Exception ex)
            {
