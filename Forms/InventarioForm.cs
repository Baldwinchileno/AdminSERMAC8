...
namespace AdminSERMAC.Forms
{
    public class InventarioForm : Form
    {
        ...
        private void AgregarButton_Click(object sender, EventArgs e)
        {
            ...
            // Rellenar la columna Fecha de Compra en VisualizarInventarioForm
            VisualizarInventarioForm visualizarInventarioForm = new VisualizarInventarioForm(sqliteService, _logger as ILogger<VisualizarInventarioForm>);
            foreach (DataGridViewRow row in visualizarInventarioForm.inventarioDataGridView.Rows)
            {
                if (row.IsNewRow) continue;
                row.Cells["FechaCompra"].Value = DateTime.Now.ToString("yyyy-MM-dd");
            }
        ...
        {
            using (var crearProductoForm = new CrearProductoForm(_logger as ILogger<CrearProductoForm>))
            {
